...
template            = '3.2.5.simple_RNN_LOSO_confidence_split.py'
...
for _key in ['past','recent']:
    for ii,((filename),df_sub) in enumerate(df_def.groupby(["filename"])):
        ...
        new_batch_script_name = os.path.join(bash_folder,f'LOO_{_key}{ii+1}')
        content = f"""#!/bin/bash
#PBS -q bcbl
#PBS -l nodes={node}:ppn={core}
#PBS -l mem={mem}gb
#PBS -l cput={cput}:00:00
#PBS -N LOSO_RNN{_key[0]}{ii+1}
#PBS -o outputs/out_RNN_{_key}_{ii+1}.txt
#PBS -e outputs/err_RNN_{_key}_{ii+1}.txt
...
python "{template.replace('.py',f'_{_key}_{ii+1}.py')}"
    """
        ...
        collections.append(f"qsub LOO_{_key}{ii+1}")
